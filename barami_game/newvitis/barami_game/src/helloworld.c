...
void note(double freq, double length, XGpio* Gpio0, XLlFifo* fifo_i2s){
	if(freq < 0){
		for(int i=0;i<48000*length;i++) {
	   		data_write(fifo_i2s,(u16)0,(u16)0);
		}
		return;
	}
	for(int i=0; i<48000*length; i++){
		right[i]=((u16) (cos((double)((i*(freq)))/48000*2*M_PI) * 32768));
		left[i]=((u16) (cos((double)((i*(freq)))/48000*2*M_PI) * 32768));
	}
...
int main(void)
{
    ...
    for(int i = 0; i<notenum; i++){
    	note(freq[i], length[i],  &Gpio0, &fifo_i2s);

    	if(XGpio_DiscreteRead(&Gpio1, 1)){
    		break;
    	}
    }
